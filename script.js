...
slider.oninput = () => {
  ...
  // 🔥 ALWAYS show submit button when slider is touched
...
// --- SLIDER --- //
guessDropdown.addEventListener("change", () => {
  const selectedIndex = parseInt(guessDropdown.value);
  slider.max = durations[selectedIndex];
...
function checkAnswer() {
  ...
  if (guessedFile === correctFileIndex) {
    ...
    message += `✅ Correct Video!<br>`;
    message += `⏱️ You were ${timeError.toFixed(2)}s off<br>`;
    message += `Actual time: ${Math.round(correctTime)}s<br>`;
    message += `🎯 Points: ${score.toFixed(1)} Gay`;

  } else {
    message += `❌ Wrong Gooner Video! It was ${correctFileIndex + 1}.mp3<br>`;
    message += `Actual time: ${Math.round(correctTime)}s<br>`;
    message += `🎯 Points: 0 Gay`;
  }
